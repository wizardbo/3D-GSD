...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            try:
                self.global_step = int(self.arg.weights[:-3].split('-')[-1])  # 修改self.global_step = int(arg.weights[:-3].split('-')[-1])
            except:
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        if epoch + 1 < self.arg.eval_start:  # eval_Start = 1
            return
...
def main():
    ...
    if p.config is not None:
        with open(p.config, 'r') as f:
            default_arg = yaml.load(f, Loader=yaml.FullLoader)  # Loader = yaml.FullLoader修改
        key = vars(p).keys()
